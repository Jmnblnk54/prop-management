        ...
        <div className="min-h-screen bg-white text-gray-900">
            ...
            <header className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur">
                <div className="flex h-14 items-center gap-3 px-3 md:px-5">
                    ...
                    {/* Quick actions (keep in header only) */}
            ...
            {/* Fixed overlay sidebar (retractable) */}
            <Sidebar collapsed={sidebarCollapsed} onToggle={toggleSidebar} />

            {/* Mobile drawer */}
            <MobileNav open={mobileOpen} onClose={() => setMobileOpen(false)} />

            {/* Main content: full viewport width; pages handle their own centering */}
            <main className="min-h-[calc(100vh-56px)] w-full px-4 md:px-6">{children}</main>
        </div>
    );
}
